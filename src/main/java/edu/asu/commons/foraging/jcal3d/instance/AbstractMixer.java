...
/* 
 * AbstractMixer defines the API that CalModel relies on for
 * blending and scheduling animations. A third party mixer must
 * implement this API in order to register itself with the
 * Model.setAbstractMixer method. The default mixer (CalMixer) is
 ...
 * AbstractMixer. However, when chosing this path, one must also
 ...
 * CalModel regardless.
 *
 * Subclassing AbstractMixer when implementing an alternate mixer
 ...
 *
 * 1) An external mixer: ignore AbstractMixer and implement a mixer
 ...
 *
 * 2) A mixer registered in cal3d : subclass AbstractMixer,
 * register it with Model.setAbstractMixer.  Avoid libraries and
 * applications that rely on the default mixer CalMixer, as returned
 * by CalModel::getMixer. CalModel::getMixer will return a null
 * pointer if Model.setAbstractMixer was called to set
 ...
 * applications can switch to it by calling Model.getAbstractMixer
...
public interface AbstractMixer {
