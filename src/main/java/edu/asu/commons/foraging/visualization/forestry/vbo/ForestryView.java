...
//	        	System.out.println("Sent a LockResourceRequest to lock the tree " + tree);
    ...
    private static class FruitTimer extends Timer {
        private final Tree tree;
        private ForagingClient client;
        
        FruitTimer(Tree tree, int delay, ForagingClient client, ActionListener listener) {
            super(delay, listener);
            this.tree = tree;
            this.client = client;
        }
    ...
    private FruitTimer createFruitFallTimer(final Woodcutter avatar, final Tree tree, long duration, int interval) {
        ...
        return new FruitTimer(tree, interval, client, new ActionListener() {
            public void actionPerformed(ActionEvent event) {
